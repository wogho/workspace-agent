...
- **Profile**: `ss-accounting-web` — 모델 계정은 `~/.config/smartstore/agy-accounts.conf` 배정(Slack 워크스페이스별 기본 계정)을 따른다. Hermes 실행은 전용 프록시 `127.0.0.1:11435/employee/ss-accounting-web`, Slack·Claw3D 대화는 같은 계정의 agy를 직접 쓴다.
- **Workspace**: `/home/ubuntu/.hermes/workspace-ss-accounting-web`
- **Role**: 회계팀 회계 웹 담당
...
- **인계선**: 환불 결과는 사용자로부터 받아 CUSTOMER_REFUND·VENDOR_REFUND로 기록
