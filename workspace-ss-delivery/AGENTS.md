...
- **Profile**: `ss-delivery` — 모델 계정은 `~/.config/smartstore/agy-accounts.conf` 배정(Slack 워크스페이스별 기본 계정)을 따른다. Hermes 실행은 전용 프록시 `127.0.0.1:11435/employee/ss-delivery`, Slack·Claw3D 대화는 같은 계정의 agy를 직접 쓴다.
- **Workspace**: `/home/ubuntu/.hermes/workspace-ss-delivery`
- **Role**: 구매팀 배송·직접전달
...
- **인계선**: 직접전달 완료 → 사용자 CS와 네이버 회계에 각각 인계
