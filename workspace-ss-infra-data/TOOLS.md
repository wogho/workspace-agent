...
## Hermes 도구 (프로필 `ss-infra-data`로 실행될 때)

켜진 도구 묶음: `file`, `web`, `skills`, `memory`, `todo`, `session_search`, `clarify`, `kanban`, `terminal`

- `file`: 기준 문서·작업 폴더 읽기와 검색. 쓰기는 자기 작업 폴더(`/home/ubuntu/.hermes/workspace-ss-infra-data`)의 초안·보고서에만 한다.
...
- `terminal`(인프라팀만): 서비스 상태·로그·백업 결과 확인 위주. 서비스 변경·배포·삭제는 사용자 승인 후에만 한다.
